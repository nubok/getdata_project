...
Next we do step "2. Extracts only the measurements on the mean and standard deviation for each measurement.". I interpreted as getting the fields having "mean()" or "std()" in its name. This interpretation explicitly means that I ignore the fields having "meanFreq()" in their name and gravityMean, tBodyAccMean, tBodyAccJerkMean, tBodyGyroMean, tBodyGyroJerkMean. Why this decision? You can read in feature_info.txt of the dataset: "meanFreq(): Weighted average of the frequency components to obtain a mean frequency". Thus meanFreq() is a more complicated average than a mean and thus not to be considered in the exercise. For a similar reason the latter variables are "Additional vectors obtained by averaging the signals in a signal window sample.". This is also not a mean of the measurements, but a mean over a signal windows. So it is principally clear which fields to extract and I only have to explain how my implementation works.
